...
load(file=paste(datadir, "DSentryALL.rdat", sep="")) #All data using REWARDED ENTRY as event with minpsth=2 and maxpsth=10 and bin=50ms. 
load(file=paste(datadir, "DSentryALLlong.rdat", sep="")) #All data using REWARDED ENTRIES LONGER THAN 2S as event with minpsth=2 and maxpsth=10 and bin=50ms.
load(file=paste(datadir, "NSentryALL.rdat", sep="")) #All data using NS ENTRY as event with minpsth=2 and maxpsth=10 and bin=50ms. 
load(file=paste(datadir, "ITIentryALL.rdat", sep="")) #All data using ITI ENTRY as event with minpsth=2 and maxpsth=10 and bin=50ms. 

load(file=paste(datadir, "ZscDSentry.rdat", sep="")) #Firingmat in Zsc using REWARDED ENTRY as event with minpsth=2 and maxpsth=10 and bin=50ms. BL: FR between sec 8 and 10
load(file=paste(datadir, "ZscDSentryLong.rdat", sep="")) #Firingmat in Zsc using REWARDED ENTRY longer than 2s  as event with minpsth=2 and maxpsth=10 and bin=50ms. BL: FR between sec 8 and 10
load(file=paste(datadir, "ZscNSentry.rdat", sep="")) #Firingmat in Zsc using NS ENTRY as event with minpsth=2 and maxpsth=10 and bin=50ms. BL: FR between sec 8 and 10
load(file=paste(datadir, "ZscITIentry.rdat", sep="")) #Firingmat in Zsc using ITI ENTRY as event with minpsth=2 and maxpsth=10 and bin=50ms. BL: FR between sec 8 and 10
...
#######################################################################
### FR AROUND TIME OF DS ENTRY - Discarding entries shorter than 2s ###
#######################################################################

#Arrange toplot based on performance (DSRR), then animal and and then firing rate to DS ENTRY LONG (ZEntryDSLong)-Long bc I discarded entries shorter than 2s
toplot$Idx <- 1:nrow(toplot)
toplot2 <- arrange(toplot, as.numeric(DSRR), desc(as.numeric(rat)), as.numeric(NSRR), as.numeric(ZEntryDSLong))
toplot2[,(ncol(toplot2)-3):ncol(toplot2)] <- round(toplot2[,(ncol(toplot2)-3):ncol(toplot2)], 1)

#I am going to use DS entry Long Firingmat in Zsc (ZEntryDSLong) as my dataset. I want to rearrange the columns (units), based on performance, I can use the toplot2$Idx
ZscData <- ZEntryDSLong
...
title("Distribution FR around time of S+ entry (>2s) per unit")
...
# Main title
mtext(text="FR around time of S+ ENTRY (>2s) throughout learning (NO INFUSION)", side=3, line=2, cex=1.2, font=2, col="gray50")

#X axis
secs <- seq(-psthmin, psthmax, by=1) #psthmin and max are objects I created in the MV neural histograms TRAINING script
secs50msBin <- seq(-psthmin, psthmax, by=0.05) #psthmin and max are objects I created in the MV neural histograms TRAINING script

divs <- seq(xmin, xmax, by=(xmax-xmin)/(length(secs)-1))
divs50msBin <- seq(xmin, xmax, by=(xmax-xmin)/(length(secs50msBin)-1))

axis(side=1, line=-1.2, at=divs, labels=secs, padj=-1, cex.axis=0.9)
mtext("Time from ENTRY (s)", side=1, line=0, at=xmin+(xmax-xmin)/2, font=2)
...
#Indicate target period and BL for analysis
StartTarget <- divs50msBin[firstBin]
EndTarget <- divs50msBin[lastBin]
StartBaseline <- divs50msBin[BLfirstBin]
EndBaseline <- divs50msBin[BLlastBin]
segments(x0=StartTarget, x1=EndTarget, y0=ncol(ZscData)+2, y1=ncol(ZscData)+2, lwd=3)
segments(x0=StartBaseline, x1=EndBaseline, y0=ncol(ZscData)+2, y1=ncol(ZscData)+2, lwd=3, col="gray80")
...
# X Axis of beh plot
axis(side=1, line=-1.2, at=c(rectMin, rectMin+(rectMax-rectMin)/2, rectMax), labels=c(minBeh, (maxBeh-minBeh)/2, maxBeh), padj=-1, cex.axis=0.9)
...
# Set limits of FR
boxplot(ZscData) #Use this boxplot to set the max limit of FR so that outliers don't skew the distribution and therefore obscure the relationship
title("Distribution of FR around time of S- entry per unit")
...
# Main title
mtext(text="FR around time of S- ENTRY throughout learning (NO INFUSION)", side=3, line=2, cex=1.2, font=2, col="gray50")
...
# Set limits of FR
boxplot(ZscData) #Use this boxplot to set the max limit of FR so that outliers don't skew the distribution and therefore obscure the relationship
title("Distribution of FR around time of ITI entry per unit")
minFR <- -4
maxFR <- 4 #Anything above this will be categorized as this value
...
# Main title
mtext(text="FR around time of ITI ENTRY throughout learning (NO INFUSION)", side=3, line=2, cex=1.2, font=2, col="gray50")
