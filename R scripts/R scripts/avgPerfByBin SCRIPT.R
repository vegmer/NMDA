avgPerfByBin <- function(binsize=10, colors=c("gray20", "gray50"), data=list(DStaskAcc, NStaskAcc), cues=c("S+", "S-"), index="Performance index", legendLocation="topleft", y_axis_label, behGraphFolder=behGraphFolder, plot=T){
        
        forPlotDF <- lapply(seq(1, length(data)), function(x){ #For each index I am interested in
                dataSel <- data[[x]]
                byRatPerBin <- sapply(seq(1, length(dataSel)), function(y){ #For each rat
                        trialIdx <- 1:length(dataSel[[y]])
                        bincuts <- seq(1, max(trialIdx), by=binsize)
                ...
                #Some rats have less bins than others (because they received less trials. Fill in with NAs the bins that they don't have, we'll ignore them later when we calculate the mean and SEM)
                maxBin <- max(sapply(byRatPerBin, length))
        ...
        if(plot==T){
                ...
                ymin=floor(min(unlist(forPlotDF)))
                ymax=ceiling(max(unlist(forPlotDF)))
                yrange <- ymax-ymin
                ...
                xaxisLab <- binsize * (1:maxBin)
                axis(side=1, at=seq(1, maxBin), labels = xaxisLab, cex.axis=1.4)
