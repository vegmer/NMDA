PerformanceFromCP <- function(relTrialMin=-100, relTrialMax=100, trialBinSize=5, typegraph="both", 
                              ...
                              csacqidx=csacqidx, idx=idx, rats=rats, alldata=alldata){
        ...
        lapply(seq(1, length(filesCum)), function(j){
                ...
                if(typegraph=="lines"){lines(x=xcoord, y=ycoord, lwd=2)} 
                if(typegraph=="points"){
                        points(x=xcoord, y=ycoord, pch=19, cex=1.5)
                        errBars(x=xcoord, y=ycoord, err=ySEM)
                }
                if(typegraph=="both"){
                        lines(x=xcoord, y=ycoord, lwd=2, col="black")
                        points(x=xcoord, y=ycoord, pch=19, cex=1.5, col="black")
                        errBars(x=xcoord, y=ycoord, err=ySEM, color="black")
                }
                ...
                #mtext(side=2, line=3, text="Latency (s)", cex=1.5, font=2)
