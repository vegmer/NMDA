...
# "DifferentTaskVersion" --> In my project, animals ran after 07/01/17 were trained with a slightly different version of the task (S+ offset happened 2s after entry if the animal entered before the end of the cue). 
#    If this parameter is TRUE, then the code will adjust based on that date to process one or the other version of the task. If this parameter is "FALSE", then the code will assume all animals were trained with the original version of the task (cue offset at the time of entry)
...
MedPCextract <- function(MovAvg="Impinged only", funcdirect=funcdirect, datafolder=datafolder, 
        ...
        for (i in(1:length(allfiles))) { 
                ...
                # For each S+, the time of cued entry or, if the animal failed to enter, NA.
                CSplusresponse <- sapply(seq(1, length(CSpluscue)), function(j){
                        CSplusentries = receptacleentries[receptacleentries > CSpluscue[j] & receptacleentries < CSpluscue[j]+cuelength]
                        if (length(CSplusentries) > 0){CSplusresponse[j] = min(CSplusentries)} else {CSplusresponse[j] = NA}
                })
                
                # Index of S+ trials that were correct trials (the animal responded to the cue)
                correctTrials <- (1:length(CSplusresponse))[!is.na(CSplusresponse)]
                
                #In July 2017 I started using a version of the task in which the cue extends 2s after correct entry, so calculating the CSplusEnd will be different after 07/01/2017
                if(DifferentTaskVersion==TRUE){
                        ...
                        if(as.Date(substr(toanalyze$info[2], 13, 20), format='%m/%d/%y')<as.Date("07/01/17", format="%m/%d/%y")){ #Before change
                                ...
                                for(j in 1:length(CSpluscue)){
                                        if(sum(j==correctTrials)>0){
                                                selTrial <- correctTrials==j
                                                CSplusEnd[j] = (toanalyze$rewarddelivery[selTrial])+2
                                        } else {
                ...
                } else { #If all animals were trained with the original version of the task (cue ended upon head entry), then set the "DifferentTaskVersion" parameter to false.
                ...
                CSminusEnd <- CSminuscue+cuelength #The end of the S- is always set to the length of the cue, bc the animal's behavior during this cue is irrelevant
                
                
                CSminusresponse <- sapply(seq(1, length(CSminuscue)), function(k){
                ...
                allCues = sort(c(CSpluscue, CSminuscue))
                allCueEnds = sort(c(CSplusEnd, CSminusEnd))
                
                ITIlength <- sapply(seq(1, length(allCues)), function(i){
                        if(i==1){a <- allCues[i]} else {
                                a <- allCues[i]-allCueEnds[i-1]
                        } 
                        a})
                
                lengthCue <- allCueEnds-allCues #If the animal responded to the S+, its duration is not 10. Make a vector with the actual length of each cue
                ...
                firstTr <- 1 # In some cases I may want to drop the first trial for some analyses. If firstTr==1, then first trial is included.
                
                #ITI pseudolatency calculation
                ITIlatency <- sapply(seq(firstTr, length(ITIlength)), function(x){
                        
                        # Define the 10s pre cue ITI window that will be used for calculating the performance index.
                ...
                #Performance index calculation (it's called "Task Accuracy" or "TA" here)
                allCuesOrder2 <- allCuesOrder; CSplusLat2 <- CSplusLat
                CSplusTA <- ITIlatency[which(allCuesOrder2==1)] - CSplusLat 
                CSminusTA <- ITIlatency[which(allCuesOrder2==2)] - CSminusLat
                
                
                #The output of this "for" loop will be a list in which each element includes summary data of the behavior of each rat on each session. I'll include a lot of the indices just in case
        ...
        # I want to add summary info of each session to csacqidx. It's not very efficient to do this with a "for" loop, I should change this
        DSperc <- c()
        NSperc <- c()
        ITItotal <- c()
        ITIperSec <- c()
        ...
        for (i in 1:length(alldata)){
                ...
                ITIrespRatio[[i]] <- sum(alldata[[i]]$ITIlatency!=10)/length(alldata[[i]]$ITIlatency)
        ...
        csacqidx["ITIrespRatio"] <- ITIrespRatio
        ...
        #ITI response ratio (whether the animal entered during that window or not)
        ITIrespRatio <- lapply(seq(1, length(rats)), function(k){
                do.call("c", lapply(seq(1, length(idx[[k]])), function(m){
                        sessIdx <- idx[[k]][m]
                        ITIlatency <- alldata[[sessIdx]]$ITIlatency
                        ITIrespRatio <- ITIlatency!=10
                        as.numeric(ITIrespRatio)
                }))
        })
        ...
        ITIlatMovingAvg <- lapply(seq(1, length(rats)), function(k){
                ...
                #and the trial in question). Same thing if the trial is within the last 11 trials.
        ...
        if(MovAvg=="Impinged only"){
                ...
                #Just for calculating how common the problem of the rat being inside the compartment in the ITI pseudolatency window is. This gives me an index of the trials in which the animals were inside the compartment at the time the critical "ITI window" started
        ...
        save(ITIrespRatio, file=paste(dataForRCumulative, "ITIrespRatio.rdat", sep=""))
}
