...
FRandPerf_Scatterplot <- function(condition="All", data=toplotSumm, graphFolder=ScatterplotFolder, CPtoo=TRUE, CPmerged=FALSE,
                                  color=c(colindx[1], colindxB[1], colindxC[1]), behIndex="PerfIndex"){
        ...
        maxY <- ceiling(max(data$ZDS, na.rm = T))
        ...
        sapply(SessIdx, function(x){
                
                dataSel <- data[data$BeforeCP==x,]
                
                if(x==1){pch=19; col=color[1]}
                
                if(CPmerged==TRUE){
                        if(x<1){pch=19; col=color[2]}
                } else {
                        if(x==0){pch=19; col=color[2]}
                        if(x==-1){pch=19; col=color[3]}
                }
                ...
                axis(side=2, at=seq(-3, 7, by=1), cex.axis=1.4, las=2)
        ...
        legend("bottomright", col=c(color), pch=19, legend=c("After CP", "CP", "Before CP"))
