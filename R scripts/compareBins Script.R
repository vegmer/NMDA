compareBins <- function(data=Diff_DSNS_NotDown_35bins, cueExcOnly=FALSE, color=colindx[1], ymin=-2, ymax=18,
        ...
        filename <- paste(graphFolder, experiment, cue,"FR trial from CP", unitSel, ".pdf", sep="_")
        pdf(file = filename)
        
        plot.new()
        plot.window(xlim=c(xmin, max(xmax)), ylim=c(ymin, ymax))
        
        FRbyUnitbyBin <- lapply(seq(1, length(uniqueBins)), function(i){
                ...
                if(cue=="S+"){eventFR <- perBin$byUnitFR}
                if(cue=="S-"){eventFR <- perBin$byUnitFR_NS}
                
                summ <- summary(eventFR)
                ...
                if(points==TRUE){
                        points(x=rep(i, nrow(perBin)), y=eventFR, pch=19, cex=0.5)
                }
        ...
        forComp1 <- FRbyUnitbyBin[c(!is.even(x=nBins))] #Bins 1, 3, 5, 7, etc.
        forComp2 <- FRbyUnitbyBin[c(is.even(x=nBins))] #Bins 2, 4, 6, 8, etc.
        ...
        Comp1 <- do.call("rbind", lapply(seq(1, length(forComp1)-1), function(n){
                ...
                data.frame(idx=bin1Idx, bin=paste(bin1Idx, " vs. ", bin2Idx, sep=""), 
                           n=paste(length(forComp1[[n]]), " vs. ", length(forComp1[[n+1]]), sep=""),
        ...
        Comp2 <- do.call("rbind", lapply(seq(1, length(forComp2)-1), function(n){
                ...
                data.frame(idx=bin1Idx, bin=paste(bin1Idx, " vs. ", bin2Idx, sep=""), 
                           n=paste(length(forComp2[[n]]), " vs. ", length(forComp2[[n+1]]), sep=""),
                           comp=paste(bin1Name1, " to ", bin1Name2, " vs. ", bin2Name1, " to ", bin2Name2, sep=""), 
                           W=w$statistic, p.val=w$p.value)
        })
        )
        ...
        if(!is.na(comparisons[1])){
                ...
                select.comp <- do.call("rbind", lapply(seq(1, length(comparisons)), function(x){
                        tocomp1<- unlist(FRbyUnitbyBin[comparisons[[x]][1]])
                        tocomp2 <- unlist(FRbyUnitbyBin[comparisons[[x]][2]])
                        ...
                        data.frame(bins=paste(comparisons[[x]][1], " vs. ", comparisons[[x]][2], sep=""), 
                                   n=paste(length(tocomp1), " vs. ", length(tocomp2), sep=""),
...
save(compareBins, file="E:/Dropbox/NMDA/R Functions/compareBins.R")
